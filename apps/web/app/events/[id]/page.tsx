...
import { CheckCircle2, Clipboard, Clock, Vote } from "lucide-react";
          ...
          <div className="space-y-6">
            ...
            {event.status === "CONFIRMED" && event.finalSlot ? (
              <Card className="border-primary/30 bg-primary/5">
                ...
                <p className="mt-1 text-2xl font-semibold">{formatSlot(event.finalSlot, event.duration)}</p>
              </Card>
            ) : null}
              ...
              <Card>
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Ranked slots refresh every 30 seconds.</p>
                  </div>
                  <Button disabled={pollMutation.isPending || event.suggestions.length === 0} onClick={() => pollMutation.mutate()}>
                    ...
                    return (
                      <div key={suggestion.id} className="rounded-lg border border-border p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <p className="font-medium">#{suggestion.rank} {formatSlot(suggestion.startTime, event.duration)}</p>
                          ...
                          <span className="text-sm font-semibold">{suggestion.score.toFixed(1)}</span>
                        </div>
                        <Progress className="mt-3" value={(suggestion.score / maxScore) * 100} />
                      </div>
                    );
                  })}
...
function formatSlot(start: string, duration: number): string {
